...
X = data
train_data1, test_data1, train_target1, test_target1 = sklearn.model_selection.train_test_split(X, target_variable, test_size=0.2, random_state=42)

# Preprocessing Pipeline for TRAIN data
# Impute "property_zipcode"
train_data1['property_zipcode'] = train_data1['property_zipcode'].astype('category')
train_data1['property_zipcode'] = train_data1['property_zipcode'].apply(lambda x: str(x).replace(" ", ""))
zipcode_dataset = pd.DataFrame({'lat': train_data1['property_lat'], 'lon': train_data1['property_lon'], 'zipcode': train_data1['property_zipcode']})
...
train_data1['property_zipcode'] = zipcode_dataset_imputed_zipcodes

# Create location variable indicating if location is in BRU or ANT
train_data1['property_lat'] = (train_data1['property_lat'] - train_data1['property_lat'].mean()) / np.std(train_data1['property_lat'])
train_data1['property_lon'] = (train_data1['property_lon'] - train_data1['property_lon'].mean()) / np.std(train_data1['property_lon'])
train_data1['location'] = train_data1['property_lat'].apply(lambda x: 1 if x < 1 else 0)
...
train_data1['property_zipcode_targetmean'] = train_data1.apply(lambda row: zips_mean[str(row['property_zipcode'])], axis=1)
train_data1['property_zipcode_freq'] = train_data1.apply(lambda row: zips_freq[str(row['property_zipcode'])], axis=1)
zips_train = train_data1['property_zipcode'].unique()
...
train_data1['property_feature_type'] = train_data1.apply(lambda x: prop_type_bins(x['property_type']), axis=1)
...
dums_train = hotOneEncode(train_data1, ['property_feature_type'])
train_data1.reset_index(drop=True, inplace=True)
dums_train.reset_index(drop=True, inplace=True)
train_data1 = pd.concat([train_data1, dums_train], axis=1)

# Encode "property_amenities"
property_amenities = train_data1['property_amenities']
...
median_bathroom = train_data1['property_bathrooms'].median()
train_data1['property_bathrooms_missing'] = train_data1['property_bathrooms'].apply(lambda x: 0 if not pd.isna(x) else 1)
train_data1['property_bathrooms'] = train_data1['property_bathrooms'].apply(lambda x: median_bathroom if pd.isna(x) else x)

median_bedroom = train_data1['property_bedrooms'].median()
train_data1['property_bedrooms_missing'] = train_data1['property_bedrooms'].apply(lambda x: 0 if not pd.isna(x) else 1)
train_data1['property_bedrooms'] = train_data1['property_bedrooms'].apply(lambda x: median_bedroom if pd.isna(x) else x)


# Preprocessing Pipeline for TEST data
# Encode "property_zipcode" for TEST data
test_data1['property_zipcode'] = test_data1['property_zipcode'].astype('category')
test_data1['property_zipcode'] = test_data1['property_zipcode'].apply(lambda x: str(x).replace(" ", ""))
test_data1['property_lat'] = (test_data1['property_lat'] - test_data1['property_lat'].mean()) / np.std(test_data1['property_lat'])
test_data1['property_lon'] = (test_data1['property_lon'] - test_data1['property_lon'].mean()) / np.std(test_data1['property_lon'])
test_zipcodes_imputed = test_data1.apply(lambda row: knn_imputer.predict([row.iloc[[12, 13]]])[0] if pd.isna(row['property_zipcode']) else row['property_zipcode'], axis=1)
test_data1['property_zipcode'] = test_zipcodes_imputed
...
test_data1['property_feature_type'] = test_data1.apply(lambda x: prop_type_bins(x['property_type']), axis=1)
dums_test = hotOneEncode(test_data1, ['property_feature_type'])
test_data1.reset_index(drop=True, inplace=True)
dums_test.reset_index(drop=True, inplace=True)
test_data1 = pd.concat([test_data1, dums_test], axis=1)

# Encode "property_amenities" for TEST data
property_amenities = test_data1['property_amenities']
for i, v in enumerate(property_amenities):
    cleaner = str(v).replace('"', "").replace("'", "").replace(" ", "").replace("/", "").replace("]", "").replace("[", "")
    property_amenities[i] = np.array(cleaner.split(','))

one_hot = pd.DataFrame(mlb.fit_transform(property_amenities), columns=mlb.classes_)
test_data1 = pd.concat([test_data1, one_hot], axis=1)

# Encode "extra" for TEST data
property_extra = test_data1['extra']
for i, v in enumerate(property_extra):
    cleaner = str(v).replace('"', "").replace("'", "").replace(" ", "").replace("/", "").replace("]", "").replace("[", "")
    property_extra[i] = np.array(cleaner.split(','))
...
test_data1['property_bathrooms_missing'] = test_data1['property_bathrooms'].apply(lambda x: 0 if not pd.isna(x) else 1)
test_data1['property_bathrooms'] = test_data1['property_bathrooms'].apply(lambda x: median_bathroom if pd.isna(x) else x)

test_data1['property_bedrooms_missing'] = test_data1['property_bedrooms'].apply(lambda x: 0 if not pd.isna(x) else 1)
test_data1['property_bedrooms'] = test_data1['property_bedrooms'].apply(lambda x: median_bedroom if pd.isna(x) else x)


# PREDICTION
# train_data1, test_data1, train_target, test_target
...
train_data1.drop(train_data1.columns.difference(keep_columns), axis=1, inplace=True)
test_data1.drop(test_data1.columns.difference(keep_columns), axis=1, inplace=True)
